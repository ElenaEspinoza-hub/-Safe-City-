...
const takePhoto = () => {
  ...
  // Reducir la imagen en el dispositivo evita subir fotos enormes y hace que
  // la revision remota empiece mucho mas rapido.
  const maxDimension = 1280
  const scale = Math.min(1, maxDimension / Math.max(width, height))
  const outputWidth = Math.max(1, Math.round(width * scale))
  const outputHeight = Math.max(1, Math.round(height * scale))
  cameraCanvas.value.width = outputWidth
  cameraCanvas.value.height = outputHeight
